...
class ComplaintsActions:
    __complaint_repository: IComplaintService
    __profile_repository: IProfileService
    __question_repository: IQuestionService
    __category_repository: ICategoryComplaintService
    ...
    async def create(
        ...
        :return:            Жалоба.
        """
        if not await self.__profile_repository.exists(id=profile_id):
            raise DoesNotExistsProfile(
                detail=f"Профиль с id={profile_id} не найден."
            )
        elif not await self.__question_repository.exists(id=question_id):
            raise QuestionDoesNotExists(
                detail=f"Вопрос с id={question_id} не найден."
            )
        elif not await self.__category_repository.exists(id=category_id):
            raise CategoryComplaintDoesNotExists(
                detail=f"Категория с id={category_id} не найдена."
            )

        complaint = await self.__complaint_repository.create(
            text=text,
    ...
    async def get_list(
        ...
        """
        offset = (page - 1) * limit
        return await self.__complaint_repository.get_list(offset, limit)
    ...
    async def get_count(self) -> int:
        ...
        :return: Число жалоб.
        """
        return await self.__complaint_repository.get_count()
    ...
    async def delete_complaint(self, pk: int) -> None:
        ...
        :return:    None.
        """
        await self.__complaint_repository.delete(pk)
...
class CategoryComplaintsActions:
    ...
    async def list(self) -> list[CategoryComplaintEntity]:
        ...
        :return: Список жалоб.
        """
        return await self.__category_repository.get_list()
