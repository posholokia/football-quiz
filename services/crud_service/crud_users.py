...
from services.storage_service.user_db.interface import ORMStatisticService, ORMProfileService
...
class StatisticsCRUD(ORMAlchemy):
    ...
    async def patch(
        ...
        new_stat = await self.storage.update_user_statistics(
            after_game_stat
        )

        new_place = await self.storage.get_user_rank(
            new_stat
        )
        ...
        updated_stats, current_profile = await self.get_updated_places(
            changed_place_stats, new_place,
        )

        for stat in updated_stats:
            await self.storage.update_user_statistics(stat)

        profile_stat = await self.storage.update_user_statistics(current_profile)
        return profile_stat
